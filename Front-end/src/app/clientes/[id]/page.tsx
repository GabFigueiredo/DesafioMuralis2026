...
import { Pencil, Trash2, User, CreditCard, Calendar, MapPin } from "lucide-react";
...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    error,
    status,
  } = useGetContactsByClientIdQuery(Number(id));
    ...
    <main className="w-full max-w-4xl mx-auto pt-8 p-10 flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        {status === "pending" ? (
          ...
          <DataTable columns={contatoColumns} data={contatos.content} />
        )}
      </div>
